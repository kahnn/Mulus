...
import sensor.NodeProfileObject;
...
//		if (dev instanceof TemperatureSensor && TemperatureSensor.EPC_MEASURED_TEMPERATURE_VALUE == (byte)(epc & 0xff)) {
//			// return local value
//			return ((TemperatureSensor) dev).getMeasuredTemperatureValue();
//		}
			...
			throws Exception
	{
		...
		EchoNode node = nodes[nindex];
		DeviceObject dev = node.getDevices()[oindex];
		dev.set().reqSetProperty((byte)(epc & 0xff), d).send();
	}

	void run(String[] args)
		throws Exception
	{
		ControllerObject ctrlObj = new ControllerObject();
		DeviceObject[] devices = { ctrlObj };
